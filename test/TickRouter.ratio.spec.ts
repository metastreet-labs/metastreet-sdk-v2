...
const TEST_NODES: LiquidityNode[] = [
  ...
  {
    tick: TickEncoder.encode({ limit: 10n * 100n, duration: 0, rate: 1, limitType: LimitType.Ratio }),
    available: 100n * 10n ** 18n,
  },
  {
    tick: TickEncoder.encode({ limit: 25n * 100n, duration: 0, rate: 0, limitType: LimitType.Ratio }),
    available: 100n * 10n ** 18n,
  },
  {
    tick: TickEncoder.encode({ limit: 50n * 100n, duration: 0, rate: 2, limitType: LimitType.Ratio }),
    available: 100n * 10n ** 18n,
  },
  {
    tick: TickEncoder.encode({ limit: 75n * 100n, duration: 0, rate: 2, limitType: LimitType.Ratio }),
    available: 100n * 10n ** 18n,
  },
...
describe('TickRouter (ratio)', function () {
  ...
  it('#_decodeNodes', function () {
    ...
    expect(router._decodeNodes(TEST_NODES)).toEqual([
      ...
      {
        tick: { limit: 75n * 100n, duration: 0, rate: 2, limitType: LimitType.Ratio },
        limit: 0n,
        available: 100n * 10n ** 18n,
      },
      {
